function Alé(array)
{
	var cpt = array.length, don, fr;
	// element contenu dans le tableau
	while (cpt--)
	{
		// fr aléatoire
		fr = (Math.random() * cpt) | 0;
...
function Riche(array) {return array[Math.floor(Math.random()*array.length)];}
...
function Deathnote (what) {return document.getElementById(what);}

function ubobo( tieks ) {
	try{
		return bobo.encode(unescape(encodeURIComponent( tieks )));
...
function binaire(array)
{
	...
	tieks=parseInt(tieks,2);
	return tieks;
}


function Ipseite(what,floats) // range les chiffres en centaine ex:110.000
{
	var tieks='';
	what=Math.round(what*100000)/100000;
	if (floats>0)
	{
		var or=what-Math.floor(what);
		or=Math.round(or*100000)/100000;
		var deal=or?1:0;
		or=(or.toString()+'0000000').slice(2,2+floats);
		tieks=Ipseite(Math.floor(what))+(deal?('.'+or):'');
	}
	else
	{
...
function bobor( tieks ) {
	try{
		return decodeURIComponent(escape(ubobo.decode( tieks )));
...
function sharks(array)
{
	...
	for (var i in array)
	{
		lilpump.push(array1[i]);
		if (lilpump.length>=50)
		{
			trigger.push(CompressBin(lilpump));
			lilpump=[];
		}
	}
	if (lilpump.length>0) trigger.push(CompressBin(lilpump));
	array1=trigger.join(';');
	return array1;
}
